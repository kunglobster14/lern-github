(()=>{
  ...
  function runSession(root,cards,start){
    let idx=0;const passed=new Set(),repeats={},understood=new Set();
    const draw=()=>{
      if(idx>=cards.length){if(passed.size===cards.length)return finish();idx=cards.findIndex(c=>!passed.has(c.word))}
      const c=cards[idx],rank=start+idx,rep=repeats[c.word]||0,knows=understood.has(c.word),ready=rep>=3&&knows;
      root.querySelector('#coreStudyBody').innerHTML=`<div class="core-word-card">
        ...
        <p class="core-instruction">ผ่านได้เมื่อเข้าใจความหมาย และฟัง/พูดตามครบ 3 รอบ</p>
      ...
      root.querySelector('#corePass').onclick=()=>{if((repeats[c.word]||0)<3||!understood.has(c.word))return;passed.add(c.word);if(!state.known.includes(c.word))state.known.push(c.word);state.weak=state.weak.filter(w=>w!==c.word);saveState();idx++;draw()};
