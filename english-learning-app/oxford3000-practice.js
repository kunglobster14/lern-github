(()=>{
  ...
  function overlay(){
    ...
    const li=levelInfo();
    root.innerHTML=`<section class="oxford-extra-panel"><header><div><h2>🧠 Oxford 3000 · Random Quiz</h2><small>${li?`สุ่มตามระดับผู้เรียน ${esc(li.cefr)}`:'สุ่มจากคำศัพท์ Oxford 3000'}</small></div><button class="oxford-extra-close" type="button">×</button></header><main id="oxfordQuizBody"><div class="core-study-loading"><div class="listen-orb">Aa</div><h3>กำลังสุ่มคำทดสอบ...</h3></div></main></section>`;
  ...
  async function openQuiz(){
    ...
    try{
      ...
      const pool=levelList(list);if(pool.length<20)throw new Error(`level_pool_too_small_${pool.length}`);
      const questions=shuffle(pool).slice(0,20);run(root,pool,questions);
  ...
  function run(root,list,questions){
    ...
    const finish=()=>{const pct=Math.round(score/questions.length*100),li=levelInfo();root.querySelector('#oxfordQuizBody').innerHTML=`<div class="oxford-quiz-finish"><div class="core-finish-icon">${pct>=80?'🏆':'📘'}</div><h2>${score}/20 คะแนน</h2><p>${pct}%${li?` · ระดับ ${esc(li.cefr)}`:''} · ${pct>=80?'ผ่านเกณฑ์ 80%':'ลองสุ่มชุดใหม่เพื่อทบทวนเพิ่มเติม'}</p><div class="lab-actions"><button class="lab-secondary" id="oxfordQuizClose">ปิด</button><button class="lab-primary" id="oxfordQuizAgain">สุ่มทดสอบใหม่</button></div></div>`;root.querySelector('#oxfordQuizClose').onclick=()=>root.remove();root.querySelector('#oxfordQuizAgain').onclick=()=>openQuiz()};draw();
  }
