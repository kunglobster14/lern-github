...
const V72_PROVIDER=window.getCurriculumV72;
...
function base(day){return typeof V72_PROVIDER==='function'?V72_PROVIDER(day)||{}:{}}
function make(day){
 ...
 const rawTitle=b.title||b.v72Title||`Lesson ${day}`,goal=b.goal||'ใช้ภาษาอังกฤษตามสถานการณ์ของบท',scenario=b.scenario||'สถานการณ์ในชีวิตประจำวัน';
 const levelNow=b.level||level(day),title=`L${day} · ${rawTitle}`;
 const objective=`L${day} · ${role.th}: ${goal}`;
 const uniqueSeed=`M${String(module).padStart(2,'0')}-L${String(day).padStart(3,'0')}-${role.key}`;
 return {
   ...b,version:VERSION,day,lessonCode:`L${day}`,module,level:levelNow,title,englishTitle:b.englishTitle||b.en||'',goal,scenario,
...
window.getCurriculumV72=day=>make(day);
})();