(()=>{
  ...
  const CACHE_KEY='myEnglishCore3000CardsV1';
  ...
  async function loadList(){
    try{
      const stored=JSON.parse(localStorage.getItem(LIST_KEY)||'null');
      if(Array.isArray(stored)&&stored.length===TARGET)return stored;
    }catch{}
    ...
    const p=plan();p.sourceReady=true;saveState();
    return out;
  }
  ...
  async function enrich(words){
    ...
    if(missing.length){
      ...
      data.cards.forEach(c=>{if(c?.word)cache[c.word]=c});
      saveCards(cache);
    }
    return words.map(w=>cache[w]).filter(Boolean);
  }
  ...
  async function openStudy(){
    ...
    try{
      ...
      const p=plan();
      const start=Math.min(Number(p.mastered)||0,TARGET-1);
      const daily=Math.max(1,Math.min(20,Number(p.daily)||12));
  ...
  function runSession(root,cards,start,list){
    ...
    const draw=()=>{
      ...
      root.querySelector('#coreAgain').onclick=()=>{
        if(!state.weak.includes(c.word))state.weak.push(c.word);
        saveState();idx=(idx+1)%cards.length;draw();
      };
    };

    const finish=()=>{
      const p=plan();p.mastered=Math.min(TARGET,(Number(p.mastered)||0)+cards.length);p.lastCompletedAt=new Date().toISOString();saveState();
      root.querySelector('#coreStudyBody').innerHTML=`<div class="core-study-finish"><div class="core-finish-icon">🏆</div><h2>ผ่านชุดวันนี้แล้ว</h2><p>คุณผ่าน ${cards.length} คำ · รวม ${p.mastered}/${TARGET} คำ</p><div class="core-finish-routine">พรุ่งนี้ระบบจะเลือกคำชุดถัดไป และคำเก่าจะกลับมาในรอบทบทวน 1 · 3 · 7 · 14 · 30 วัน</div><button class="primary-btn" id="coreDone">กลับหน้าแรก</button></div>`;
      root.querySelector('#coreDone').onclick=()=>{root.remove();go('home')};
    };
    draw();
  }
