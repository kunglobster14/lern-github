(()=>{
  ...
  function syncProfile(){const sel=document.querySelector('#learnerLevelSelect'),i=info();if(sel&&i)sel.value=i.id;const help=document.querySelector('#learnerLevelHelp');if(help&&i)help.textContent=`${i.label} · ${i.cefr} — ${i.description} · เปลี่ยนระดับได้ตลอดโดยความคืบหน้าเดิมไม่ถูกรีเซ็ต`}
  function badge(){
    ...
    const i=info();b.textContent=i?`ระดับ ${i.cefr}`:'เลือกระดับ';b.title=i?`${i.label} · ${i.description}`:'เลือกระดับการเรียน';
  }
  function markRoadmap(){
    const i=info();document.querySelectorAll('[data-path-level]').forEach(b=>{b.classList.remove('learner-recommended');b.querySelector('.learner-rec-tag')?.remove();if(i&&i.roadmap.includes(b.dataset.pathLevel)){b.classList.add('learner-recommended');const tag=document.createElement('em');tag.className='learner-rec-tag';tag.textContent=b.dataset.pathLevel===i.start?'เริ่มตรงนี้':'แนะนำ';b.appendChild(tag)}});
    const road=document.querySelector('#learningRoadmap');if(!road)return;let note=road.querySelector('.learner-roadmap-note');if(!note){note=document.createElement('div');note.className='learner-roadmap-note';const track=road.querySelector('.roadmap-track');track?.insertAdjacentElement('afterend',note)}if(i)note.innerHTML=`<b>ระดับของคุณ: ${esc(i.label)} · ${esc(i.cefr)}</b><span>แนะนำให้เริ่มที่ ${esc(i.start)} แต่บทที่เรียนจบและเปอร์เซ็นต์เดิมยังคงเหมือนเดิมทั้งหมด</span>`;else note.textContent='เลือกระดับผู้เรียนเพื่อให้ระบบแนะนำจุดเริ่มที่เหมาะสม';
  }
  ...
  const observer=new MutationObserver(()=>{if(!document.documentElement.classList.contains('account-locked')){applyUI();firstChoice()}});observer.observe(document.body,{childList:true,subtree:true});
